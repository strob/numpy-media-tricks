...
KEYMODS= dict([(getattr(sdl2, X), X.split("_")[-1]) for X in dir(sdl2) if X.startswith("KMOD_")])

def _get_keymods(mod):
    return dict([(kval,True) for kmod, kval in KEYMODS.items() if mod & kmod])

class ArrayUI:
    def __init__(self, size=(320, 240), in_size=(320,240),
    ...
    def _init_video(self):
        sdl2.ext.init()
        self._win = sdl2.ext.Window(self.name, size=self.size, flags=(sdl2.SDL_WINDOW_FULLSCREEN_DESKTOP if self.fullscreen else None))
        self._win.show()
        # TODO: determine (or override?) color-order
        # on linux, this seems to be BGRA, while on mac it's ARGB
        self._v = sdl2.ext.pixels2d(self._win.get_surface()).T.view(np.uint8).reshape((self.size[1], self.size[0], 4))
    ...
    def _next_video_in_fr(self):
        in_fr = np.fromstring(self._v_in.stdout.read(self.in_size[0]*self.in_size[1]*3), dtype=np.uint8)
        try:
    ...
    def _handle_event(self, ev):
        if hasattr(self, "mouse_in"):
            if ev.type == sdl2.SDL_MOUSEMOTION:
                ...
                self.mouse_in("mouse-button-release", ev.motion.x, ev.motion.y, ev.button)
        if hasattr(self, "keyboard_in"):
            if ev.type == sdl2.SDL_KEYDOWN:
                ...
                self.keyboard_in("key-press", KEYMAP[ev.key.keysym.sym], _get_keymods(ev.key.keysym.mod))
            if ev.type == sdl2.SDL_KEYUP:
                self.keyboard_in("key-release", KEYMAP[ev.key.keysym.sym], _get_keymods(ev.key.keysym.mod))
        # else:
...
class TestUI(ArrayUI):
    ...
    def audio_out(self, a):
        a[:,0] = 2**12 * np.sin(np.linspace(0,np.pi*2*15, num=len(a), endpoint=False))
        a[:,1] = 2**12 * np.sin(np.linspace(0,np.pi*2*14, num=len(a), endpoint=False))
